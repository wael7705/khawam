...
### حماية قاعدة البيانات (حد اتصالات + pg-boss)

**المشكلة:** كل طلب قد يفتح اتصال Postgres؛ مع الذروة تتراكم الجلسات (`pg_temp_*`) وقد تُرهق القاعدة.

**ما يعمل على Railway الآن (بدون خدمة إضافية):**

1. **حد اتصالات Prisma** — `DB_CONNECTION_LIMIT` (افتراضي 10) لكل نسخة تطبيق.
2. **اتصال مباشر منفصل** — `DIRECT_DATABASE_URL` لـ migrate/seed و pg-boss فقط.
3. **pg-boss** — طابور مهام على Postgres (بدون Redis): أرشفة الطلبات وتنظيف الملفات المؤقتة.

**متغيرات khawam (الإنتاج الحالي):**

| المتغير | القيمة |
|---------|--------|
| `DATABASE_URL` | من Postgres (شبكة خاصة) |
| `DIRECT_DATABASE_URL` | نفس Postgres مباشرة |
| `PGBOUNCER_ENABLED` | `false` |
| `DB_CONNECTION_LIMIT` | `10` |
| `QUEUE_POOL_MAX` | `3` |

**PgBouncer كبنية اختيارية** — لـ VPS أو Docker عند الحاجة فقط (`deploy/pgbouncer/` و `docker-compose`). ليس مطلوباً كخدمة منفصلة على Railway للتشغيل اليومي.

**فحص الصحة والضغط:**

```bash
curl https://khawam-production.up.railway.app/api/health
# queue: ok | pgbouncer: false
