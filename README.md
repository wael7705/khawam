...
1. **تهيئة الملفات للرفع:** العميل يجهّز الطلب كـ `multipart/form-data` (بدون تعيين هيدر `Content-Type` يدوياً).
2. **فحص الملفات (مرحلة لاحقة):** يُفضّل فحص الملفات على الخادم للتأكد من الخلو من الفيروسات أو الهجمات (مثلاً باستخدام ClamAV) قبل الحفظ.
3. **رفع الملفات للسيرفر:** استقبال الـ stream، التحقق من الامتداد والحجم والتوقيع (magic bytes)، ثم حفظ الملف في المسار المخصّص. انظر `backend/src/shared/plugins/upload.plugin.ts`.
...
- **اختبار التحميل:** `pnpm test:load` (يتطلب [k6](https://k6.io)). من جذر المشروع: `k6 run load-tests/load.js`؛ مع عنوان السيرفر: `k6 run -e BASE_URL=http://localhost:8000 load-tests/load.js`. المسارات المُختبرة: `GET /api/services` و `POST /api/orders`.
...
- **التحميل:** اختبارات التحميل بـ k6: من جذر المشروع `pnpm test:load` أو `k6 run load-tests/load.js` (يتطلب تثبيت [k6](https://k6.io)). لرفع الحمل: `k6 run --vus 50 --duration 60s load-tests/load.js`.
...
**النشر بدون Railway (VPS أو منصة أخرى):** استخدم `backend/Dockerfile` بجذر المستودع كـ build context: `docker build -f backend/Dockerfile -t khawam-backend .`. لمجاورة staging/production مع Docker و Nginx استخدم السكربتات والإعدادات في مجلد `deploy/` (docker-compose، nginx، env، scripts).
...
| المتغير | مطلوب | الوصف |
|---------|-------|--------|
| `DATABASE_URL` | نعم | اتصال التطبيق (عبر PgBouncer في الإنتاج، أو Postgres مباشرة محلياً) |
| `DIRECT_DATABASE_URL` | نعم* | اتصال مباشر بـ Postgres للمهاجرات و pg-boss. *محلياً = نفس `DATABASE_URL` |
...
### حماية قاعدة البيانات (PgBouncer + pg-boss)

**المشكلة:** كل طلب HTTP أو Socket قد يفتح اتصال Postgres؛ مع إعادة النشر أو ذروة الطلبات تتراكم الجلسات (`pg_temp_*`) وقد تُرهق قاعدة البيانات.

**الحل:**

1. **PgBouncer** — طبقة تجميع اتصالات بين التطبيق و Postgres (وضع `transaction`). ملفات الإعداد في `deploy/pgbouncer/`.
2. **pg-boss** — طابور مهام مبني على Postgres (بدون Redis): أرشفة الطلبات وتنظيف الملفات المؤقتة تُجدول عبر الطابور بدلاً من cron داخل العملية.

**على Railway (موصى به):**

1. أضف خدمة **PgBouncer** من `deploy/pgbouncer/Dockerfile`.
2. على خدمة **pgbouncer**: `DATABASE_URL=${{Postgres.DATABASE_URL}}`، `POOL_MODE=transaction`، `MAX_CLIENT_CONN=200`، `DEFAULT_POOL_SIZE=15`.
3. على خدمة **khawam**:
   - `DATABASE_URL` → رابط PgBouncer الداخلي (`*.railway.internal`)
   - `DIRECT_DATABASE_URL=${{Postgres.DATABASE_URL}}`
   - `PGBOUNCER_ENABLED=true`
   - `DB_CONNECTION_LIMIT=10`
4. **Prisma** (`db push` / seed) و **pg-boss** يستخدمان `DIRECT_DATABASE_URL` تلقائياً.

**محلياً:** اترك `PGBOUNCER_ENABLED=false` و `DIRECT_DATABASE_URL` = `DATABASE_URL`. مع Docker استخدم `deploy/docker-compose.production.yml` (يتضمن pgbouncer).

**فحص الصحة:** `GET /api/health` يعرض `queue` و `pgbouncer`.
