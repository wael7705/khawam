...
import type { OrderListItem, OrderDetail, OrderItemDetail, OrderStatusHistoryItem } from '../types/order';
import { MyOrdersFilters, type StatusFilter } from '../components/my-orders/MyOrdersFilters';
...
import { OrderTrackingHero } from '../components/my-orders/OrderTrackingHero';
...
export function MyOrders() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setStatusUpdateMessage(null), 5000);
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      setError(locale === 'ar' ? 'تعذر تحميل الطلبات' : 'Failed to load orders');
      setOrders([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadOrderDetail = useCallback(async (orderId: string) => {
    const [detailRes, historyRes] = await Promise.all([
      ...
      ordersAPI.getStatusHistory(orderId),
    ]);
    setSelectedOrder(normalizeOrderDetail(detailRes.data as Record<string, unknown>));
  ...
  }, [openOrderNumberFromState, loading, orders, navigate, location.pathname, loadOrderDetail]);
  ...
  const whatsappHref = buildWhatsAppUrl(
    locale === 'ar' ? 'مرحباً، أريد متابعة طلبي' : 'Hi, I want to follow up on my order',
  );
  ...
  return (
    <div className="my-orders-page">
      ...
      <OrderTrackingHero
        title={myOrdersLabels.title}
        subtitle={myOrdersLabels.subtitle}
        liveLabel={myOrdersLabels.liveUpdates}
        stats={orderStats}
        statsLabels={{
          orders: myOrdersLabels.ordersCount,
      ...
      <section className="my-orders section my-orders__body-section">
        <div className="container">
          {loading ? (
            <div className="my-orders__state my-orders__state--loading">
              <div className="order-page__spinner" style={{ width: 40, height: 40, borderWidth: 3 }} />
              <p>{myOrdersLabels.loading}</p>
            </div>
          ) : error ? (
      ...
      <a
        ...
        className="my-orders-fab"
        aria-label={myOrdersLabels.whatsappQuick}
      >
        <WhatsAppIcon size={26} />
        <span>{myOrdersLabels.whatsappQuick}</span>
      </a>

      {(selectedOrder || detailLoading) && (
        <div className="my-orders-overlay" onClick={closeDetail}>
          <aside className="my-orders-drawer my-orders-drawer--cinematic" onClick={(e) => e.stopPropagation()}>
            {detailLoading ? (
              ...
              <MyOrderDetailDrawer
                ...
                labels={{
                  ...
                  trackJourney: myOrdersLabels.trackJourney,
                  noHistory: myOrdersLabels.noHistory,
                }}
                onClose={closeDetail}
